...
pub async fn credit_capping<F, C, G, T>(
    ctx: C,
...
async fn credit_capping_max_one<F, C, G, T>(
    ctx: C,
    input: &[MCCreditCappingInputRow<F, T>],
...
async fn mask_source_credits<F, C, G, T>(
    input: &[MCCreditCappingInputRow<F, T>],
    ctx: C,
) -> Result<Vec<T>, Error>
where
...
async fn report_level_capping<F, C, G, T>(
    ctx: C,
    original_credits: &[T],
    cap: u32,
) -> Result<Vec<T>, Error>
...
async fn credit_prefix_sum<'a, F, C, G, T, I>(
    ctx: C,
    input: &[MCCreditCappingInputRow<F, T>],
    original_credits: I,
) -> Result<Vec<T>, Error>
...
async fn is_credit_larger_than_cap<F, C, G, T>(
    ctx: C,
...
async fn propagate_overflow_detection<F, C, G, T>(
    ctx: C,
    input: &[MCCreditCappingInputRow<F, T>],
...
async fn compute_final_credits<F, C, G, T>(
    ctx: C,
    input: &[MCCreditCappingInputRow<F, T>],
    prefix_summed_credits: &[T],
    exceeds_cap_bits: &[T],
