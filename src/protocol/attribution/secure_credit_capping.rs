    ...
    T: LinearSecretSharing<F> + BasicProtocols<C, F>,
{
    ...
    //
    // Step 2. Cap each user's credit to `cap`
    //
    let capped_credits = initial_capping(ctx.clone(), &original_credits, cap).await?;

    //
    // Step 3. Compute user-level reversed prefix-sums
    //
    let prefix_summed_credits =
        credit_prefix_sum(ctx.clone(), input, capped_credits.iter(), cap).await?;
    ...
    //
    // Step 5. Compute prefix-or of `exceeds_cap_bits`
    //
    ...
    let prefix_or_exceeds_cap_bits =
        prefix_or_binary_tree_style(ctx.clone(), &helper_bits, &exceeds_cap_bits).await?;
...
async fn initial_capping<F, C, T>(ctx: C, original_credits: &[T], cap: u32) -> Result<Vec<T>, Error>
where
    F: PrimeField,
    C: Context + RandomBits<F, Share = T>,
    T: LinearSecretSharing<F> + BasicProtocols<C, F>,
{
    ...
    let exceeds_cap_bits = is_credit_larger_than_cap(ctx.clone(), original_credits, cap).await?;

    let first_capping_ctx = ctx
        .narrow(&Step::FirstCapping)
    ...
    try_join_all(
        original_credits
            ...
            .map(|(i, (original_credit, exceeds_cap_bit))| {
                let record_id = RecordId::from(i);
                let c = first_capping_ctx.clone();
                let share_of_cap = share_of_cap.clone();
                async move {
                    if_else(
                        c,
                        record_id,
    ...
    I: Iterator<Item = &'a T>,
{
    ...
    let mut credits = capped_credits.cloned().collect::<Vec<_>>();
...
impl AsRef<str> for Step {
    fn as_ref(&self) -> &str {
        match self {
            ...
            Self::FirstCapping => "first_capping",
