...
struct ContextInner<'a, F: Field + ExtendableField, G: Gate> {
    prss: &'a PrssEndpoint,
    gateway: &'a Gateway<G>,
...
impl<'a, F: Field + ExtendableField, G: Gate> ContextInner<'a, F, G> {
    fn new(
...
#[async_trait]
impl<'a, F, G, T, TM, U, UM> UpgradeToMalicious<(T, U), (TM, UM)>
...
#[async_trait]
impl<F, G, T, M> UpgradeToMalicious<Vec<T>, Vec<M>> for UpgradeContext<'_, F, G, NoRecord>
where
    F: Field + ExtendableField,
    G: Gate,
    T: Send + 'static,
...
#[async_trait]
impl<'a, F, G, T, M> UpgradeToMalicious<Vec<Vec<T>>, Vec<Vec<M>>>
    for UpgradeContext<'a, F, G, NoRecord>
where
    F: Field + ExtendableField,
    G: Gate,
...
#[cfg(test)]
#[async_trait]
impl<'a, F, G, T, M> UpgradeToMalicious<T, M> for UpgradeContext<'a, F, G, NoRecord>
where
    F: Field + ExtendableField,
    G: Gate,
    T: Send + 'static,
